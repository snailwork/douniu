...
// 负责人：李启波（marcyli）
//============================================================
- ( NSMutableDictionary *)sendPay_demo:(NSDictionary *)dict
{

    //订单标题，展示给用户
    NSString *order_name    = @"牵手德州筹码";
//    //订单金额,单位（分）
...
//    NSString *orderno   = [NSString stringWithFormat:@"%ld",time(0)];
    ...
    [packageParams setObject: [dict objectForKey:@"notify_url"]        forKey:@"notify_url"];  //支付结果异步通知
    [packageParams setObject: [dict objectForKey:@"orderid"]           forKey:@"out_trade_no"];//商户订单号
    [packageParams setObject: @"196.168.1.1"    forKey:@"spbill_create_ip"];//发器支付的机器ip
    [packageParams setObject: [dict objectForKey:@"price"]       forKey:@"total_fee"];       //订单金额，单位为分
